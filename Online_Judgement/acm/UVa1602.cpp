...
void generate() 
{
	...
	for(int n = 2; n <= maxn; n++) 
	{
		for(set<Polyomino>::iterator p = poly[n-1].begin(); p != poly[n-1].end(); ++p)
			...
			{
				Cell newc(c->x + dx[dir], c->y + dy[dir]);/*crate a new cell */
				if(p->count(newc) == 0) check_polyomino(*p, newc);/*if current newc is not in set and check its variant*/
			}
	}
