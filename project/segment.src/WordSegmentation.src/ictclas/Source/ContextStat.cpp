...
bool CContextStat::Load(char *sFilename)
{
    ...
    ReadNums=fread(&m_nTableLen,sizeof(m_nTableLen),1,fp);//write the table length
    if(m_pSymbolTable!=NULL)
        delete [] m_pSymbolTable;
    m_pSymbolTable=new int[m_nTableLen];//new buffer for symbol
    ReadNums=fread(m_pSymbolTable,sizeof(int),m_nTableLen,fp);//write the symbol table
    //free exists items
    while(!feof(fp))
    {
        //Read the context
        ...
        ReadNums=fread(&pCur->nTotalFreq,sizeof(int),1,fp);
        pCur->aTagFreq=new int [m_nTableLen];
        ReadNums=fread(pCur->aTagFreq,sizeof(int),m_nTableLen,fp);//the every POS frequency

        pCur->aContextArray=new int *[m_nTableLen];
        for(int i=0; i<m_nTableLen; i++)
        {
            pCur->aContextArray[i]=new int[m_nTableLen];
            ReadNums=fread(pCur->aContextArray[i],sizeof(int),m_nTableLen,fp);
        }
        if(pPre==NULL)
            m_pContext=pCur;
        else
