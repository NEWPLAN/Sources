...
/*本例子基于libexif-0.6.20/contrib/examples/write-exif.c 修改而来，在图片中添加gps的经玮度信息*/
/*相关exif信息 请参考 http://www.sno.phy.queensu.ca/~phil/exiftool/index.html*/
...
int main(int argc, char **argv)
{
    ...
    ExifEntry *entry;
    /*alloc a new memory for exif data*/
    ...
    entry->format = EXIF_FORMAT_ASCII;
    entry->components = 19;
    memcpy(entry->data,"2012-12-11 10:00:00",19);
    ...
    printf("%x %x %x %x\n",exif_header[0],exif_header[1],exif_header[2],exif_header[3]);
    printf("%x %x\n",(exif_data_len+2) >> 8,(exif_data_len+2) & 0xff);
    /* Write EXIF header */
...
errout:
    ...
    free(exif_data);
    exif_data_unref(exif);
    FILE * ddp = fopen("1.jpg","wb");
    fwrite(image_jpg,image_jpg_len,1,ddp);
    fclose(ddp);
    return rc;
