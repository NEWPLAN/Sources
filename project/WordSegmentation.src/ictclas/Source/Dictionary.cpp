...
bool CDictionary::Load(char *sFilename,bool bReset)
{
    ...
    for(i=0; i<CC_NUM; i++)
    {
        ReadNums=fread(&(m_IndexTable[i].nCount),sizeof(int),1,fp);
        ...
        {
            ReadNums=fread(nBuffer,sizeof(int),3,fp);
            m_IndexTable[i].pWordItemHead[j].sWord=new char[nBuffer[1]+1];
            if(nBuffer[1])//String length is more than 0
            {
                ReadNums=fread(m_IndexTable[i].pWordItemHead[j].sWord,sizeof(char),nBuffer[1],fp);
            }
