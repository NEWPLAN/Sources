...
bool CSegment::IsYearTime(char *sNum)
{
    ...
    if(IsAllNum((unsigned char *)sNum)&&(nLen>=6||nLen==4&&CC_Find("５６７８９",sTemp)))
        return true;
    if(GetCharCount((char*)"零○一二三四五六七八九壹贰叁肆伍陆柒捌玖",sNum)==(int)nLen/2&&nLen>=3)
        return true;
    if(nLen==8&&GetCharCount((char*)"千仟零○",sNum)==2)//二仟零二年
        return true;
    if(nLen==2&&GetCharCount((char*)"千仟",sNum)==1)
        return true;
    if(nLen==4&&GetCharCount((char*)"甲乙丙丁戊己庚辛壬癸",sNum)==1&&GetCharCount((char*)"子丑寅卯辰巳午未申酉戌亥",sNum+2)==1)
        return true;
    return false;
}
