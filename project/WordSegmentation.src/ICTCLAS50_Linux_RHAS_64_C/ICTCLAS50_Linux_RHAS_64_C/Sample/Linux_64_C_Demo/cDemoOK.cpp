...
void testImportUserDict()
{
	...
	const char* pszDictBuffer="中科院@@nr;分词 v;系统@@adj;"; //字符串形式导入用户词典
	int nLen=strlen(pszDictBuffer);
	printf("newplan come %d\t%s\n",nLen,pszDictBuffer);
	unsigned int nItems=ICTCLAS_ImportUserDict (pszDictBuffer, nLen,CODE_TYPE_UNKNOWN);
	printf("newplan come here line 108\n");
