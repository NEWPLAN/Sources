...
void aes_key_setup( const BYTE key[], WORD w[] )
{
    int	Nb = 4, Nr = 10, Nk = 4, idx;
...
void aes_encrypt( const BYTE in[], BYTE out[], const WORD key[] )
{
    int	i;
    BYTE	state[4][4];
...
WORD main( WORD argc, BYTE *argv[] )
{
    ...
    BYTE key[16] =
    {
        0x3A, 0xE1, 0x15, 0x62, 0xA8, 0xF3, 0xC7, 0x1A, 0x2B, 0xF6, 0xDF, 0xA1, 0x50, 0x9B, 0xCA, 0xF1
    };
    ...
    aes_key_setup( key, key_schedule );
    aes_encrypt( plaintext, enc_buf, key_schedule );
