...
void chafen::aesC()
{
	...
	ofstream in;
	in.open("aes.txt.csv", ios::trunc);
	int counts = 0;
	int max = 0;
	int ik, jk;
	in <<  /*setw(2) << setfill('0') << hex <<*/ "in\\out" << ',';
	for ( int i = 0; i < 256; i++)
		in << /*"0x" << setw(2) << setfill('0') <<*/ hex << i << ',';
	in << endl;
	for ( int i = 0; i < 256; i++)
	{
		in << /*"0x" << setw(2) <<*/ hex << i << ',';
	...
	in << endl;
	cout << "max=" << max << " all counts=" << counts << " average= " << counts / 256.0 / 256.0 << endl;
	cout << ik << "  " << jk << ASResult[0][0] << endl;
