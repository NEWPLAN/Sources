...
//http://www.doc88.com/p-77240557845.html

unsigned char *full_array2(int n)//生成0―n的随机全排列
{
	int i, j, l = 0, m = 300, flag;
	int multis = 0;
...
static const unsigned char SboxTablewww[16][16] =
{
...
{
	0x38, 0x9c, 0x46, 0x77, 0x4a, 0xd0, 0xae, 0x75, 0xed, 0x4d, 0x44, 0xcd, 0xd2, 0x11, 0xdd, 0x92,
...
static const unsigned char boxs[16 * 16] =
{
	0x38, 0x9c, 0x46, 0x77, 0x4a, 0xd0, 0xae, 0x75, 0xed, 0x4d, 0x44, 0xcd, 0xd2, 0x11, 0xdd, 0x92,
	0x13, 0xdf, 0x01, 0xbc, 0x17, 0x92, 0xef, 0x1d, 0xb5, 0xb2, 0xe0, 0x97, 0x9c, 0x45, 0x90, 0x8b,
	0xf2, 0xe7, 0xfb, 0x94, 0x3b, 0x1d, 0x64, 0x76, 0x7d, 0xe7, 0x9c, 0x51, 0xa1, 0x94, 0x81, 0x1b,
...
void delta(unsigned char* abs, int len)
{
	int i, j;
	unsigned char temp_dst1, temp_dst2, xor_src, xor_res;
	for (i = 0; i < len; i++)
	{
		for (j = 0; j < len; j++)
		{
			temp_dst1 = boxs[i];
			temp_dst2 = boxs[j];
			xor_src = (unsigned char)i ^ j;
			xor_res = (unsigned char)temp_dst1 ^ temp_dst2;
			record[xor_src][xor_res].p[record[xor_src][xor_res].counts].src = i;
			record[xor_src][xor_res].p[record[xor_src][xor_res].counts].dst = j;
			record[xor_src][xor_res].counts++;
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	// 		for(j=0;j<256;j++)
	// 		{
	// 			if(record[i][j].counts!=0)
	// 			{
	// 				cccc++;
	// 				int k=0;
	// 				printf("%d\n",record[i][j].counts);
	// 				// for(k=0;k<record[i][j].counts;k++)
	// 				// {
	// 				// 	printf("k=%d,src=%3x,dst=%3x,src^dst=%3x,boxs_src^boxs_dst=%3x\n",k,record[i][j].p[k].src,record[i][j].p[k].dst,i,j);
	// 				// }
	// 			}
	// 			//printf("%3d",record[i][j].counts);
	// 		}
	// 		//putchar('\n');
	// 	}
	// 	printf("%d\n",cccc);
